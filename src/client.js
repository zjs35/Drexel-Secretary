...
function updateTeacherDropdown() {
		...
		$.ajax({
			...
			success : function(msg){
				...
				//if(namelist.length != 1){
					for(var j = namelist.length; j >0; j--){	
   ...
   //     $('#name').html(response);
	...
	function getCalendar() {
		...
		console.log(name[name.selectedIndex].id);
		var id = name[name.selectedIndex].id;
		var serurl = "teacherselect?id="+id;
		console.log(serurl);
		...
		$.ajax({
			...
			url  : serurl,
			dataType: "json",
			<!--if responce is recieved then it prints out the desired info into the empty div-->
			success : function(msg){ 
				console.log(msg);
				document.getElementById("emptydiv").innerHTML+= msg;
			},
			error: function(jgXHR, textStatus,errorThrown){
				alert("Error: " + textStatus+ " " + errorThrown);
			}
